...
class ResCompany(models.Model):
    ...
    additional_identification_scheme = fields.Selection(
        related='partner_id.additional_identification_scheme', readonly=False)
    additional_identification_number = fields.Char(
        related='partner_id.additional_identification_number', readonly=False)
    ...
    production_csid_validity = fields.Datetime("PCSID Expiration", help="Production CSID expiration date",
                                                       compute="_compute_production_csid_validity", store=True)
    ...
    @api.depends('digital_certificate')
    def _compute_production_csid_validity(self):
        """
            Compute the expiration date of the Production certificate
        """
        for rec in self:
            rec.production_csid_validity = False
            if rec.digital_certificate:
                rec.production_csid_validity = self._get_pcsid_validity(rec.digital_certificate)
    ...
    def _generate_csr(self):
        ...
        x509_alt_names_extension = x509.SubjectAlternativeName([
            x509.DirectoryName(x509.Name([
                ...
                x509.NameAttribute(ObjectIdentifier('2.5.4.4'), company_id.serial_number),
                ...
                x509.NameAttribute(ObjectIdentifier('2.5.4.15'),
                                   company_id.business_category or 'Other'),
            ]))
        ])
        ...
        private_key = load_pem_private_key(base64.b64decode(self.pem_private_key), password=None, backend=default_backend())
    ...
    def _get_zatca_datetime(self, timestamp):
        return fields.Datetime.context_timestamp(self.with_context(tz='Asia/Riyadh'), timestamp)

    def _get_pcsid_validity(self, PCSID):
        """
            Return PCSID expiry date
        """
        b64_decoded_pcsid = base64.b64decode(PCSID)
        x509_certificate = load_der_x509_certificate(base64.b64decode(b64_decoded_pcsid.decode()), default_backend())
    ...
    def getComplianceCSID(self):
        ...
        request_data = {
            'body': json.dumps({"csr": self.encoded_csr.decode()}),
            'header': {
    ...
    def action_zatca_renewal_company_step(self):
        time_now = self._get_zatca_datetime(fields.Datetime.now())
        if not self._get_zatca_datetime(self.production_csid_validity) < time_now:
    ...
    def callApiComplianceChecks(self, xml_string):
        xml_string, digest, QR, inv_uuid = self.env["account.move"].signe_xml(
    ...
    def complianceChecks(self):
        if self.xml_file:
            ...
            if response.status_code == 200:
                ...
                title = "Warning"
                message = "Submitted invoice is invalid"
                ...
                notification_type = "danger"
                title = "Warning"
                message = "Service faces internal errors"
        else:
            notification_type = "warning"
            title = "Warning"
            message = "Empty XML Invoice!"
        notification = {
    ...
    def onboardingComplianceChecks(self):
        ...
        for key, temp in templates.items():
            xml_string = self.env["ir.qweb"]._render("l10n_sa_e-invoice.%s" % temp, template_values)
            xml_string, response = self.callApiComplianceChecks(xml_string)
            res = response
            if response:
